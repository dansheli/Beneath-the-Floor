...
{
    public class LogisticsRobotMarker : MonoBehaviour
    {
        [Header("General")]
        [SerializeField] private Color markerColor = new Color(0.2f, 0.85f, 0.6f, 1f); // green/teal
        ...
        private void LateUpdate()
        {
            ...
            if (controller != null
                && (controller.CurrentState == LogisticsState.Docked
                 || controller.CurrentState == LogisticsState.Carried))
            {
                SetState(MarkerState.Hidden);
                return;
            }
            ...
            if (dist < hideDistance) { SetState(MarkerState.Hidden); return; }
        ...
        private void UpdateOnScreen(Vector3 screenPos)
        {
            ...
            rt.localScale = Vector3.one * Mathf.Lerp(1f, pulseScale, t);
        ...
        private void UpdateOffScreen(Vector3 screenPos, bool behind, float distMeters)
        {
            ...
            float scaleToEdge = absX / halfW > absY / halfH
                ? halfW / Mathf.Max(absX, 0.001f)
                : halfH / Mathf.Max(absY, 0.001f);
        ...
        {
            GameObject canvasObj = new GameObject("LogisticsMarkerCanvas");
            canvasObj.transform.SetParent(transform, false);
            uiCanvas = canvasObj.AddComponent<Canvas>();
            uiCanvas.renderMode = RenderMode.ScreenSpaceOverlay;
            uiCanvas.sortingOrder = 92;
        ...
        private static Texture2D CreateDiamondTexture(int size)
        {
            ...
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    float d = dx + dy;
                    if (d < radius - 1f) tex.SetPixel(x, y, Color.white);
                    else if (d < radius + 1f) tex.SetPixel(x, y, new Color(1, 1, 1, Mathf.Clamp01(1f - (d - (radius - 1f)) / 2f)));
